...
class ReportsController:
    ...
    def get_dashboard_metrics(self):
        ...
        with get_db_session() as db:
            ...
            sales = db.query(Sale).filter(func.date(Sale.date) == today_str).all()
            ...
            # Near Expiry (< 30 days)
            future_30_days = datetime.date.today() + datetime.timedelta(days=30)
            expiry_cutoff = future_30_days.strftime('%Y-%m-%d')
    ...
    def get_sales_trend(self, days=7):
        ...
        with get_db_session() as db:
            ...
            for i in range(days - 1, -1, -1):
                ...
                sales = db.query(func.sum(Sale.total_amount)).filter(func.date(Sale.date) == target_date).scalar()
